...
const liveResults = ref<HarnessRunResult['results']>([])
const runHistory = ref<HarnessRunResult[]>([])
const selectedRunId = ref<string>('')
const rerunOverrides = ref<Record<string, HarnessRunResult['results'][number]>>({})
...
const displayedResults = computed(() => {
  ...
  return selectedSuite.value.tests.map(
    (test): HarnessRunResult['results'][number] =>
...
function syncRunHistoryForActiveServer(): void {
  ...
  const savedRun = serverStore.getHarnessTestResults(activeServerId)
  if (!savedRun) {
    runHistory.value = []
    selectedRunId.value = ''
    return
...
function resolveSelectionForActiveServer(suiteTests: TestCaseDefinition[]): string[] {
  ...
  const serverSelection = serverStore.getHarnessTestSelection(serverId).filter((id) => suiteTestIds.has(id))
...
function clearHistory(): void {
  const history = clearHarnessRunHistory()
...
async function runHarness(): Promise<void> {
  ...
  const activeServerId = serverStore.activeServer?.id
  if (activeServerId) {
    serverStore.saveHarnessConfig(activeServerId, {
      sidecarUrl: sidecarUrl.value.trim() || undefined
    })
  }
  ...
  try {
    ...
    const history = appendHarnessRun(run)
    runHistory.value = history.runs
    selectedRunId.value = run.runId

    if (activeServerId) {
      serverStore.saveHarnessTestResults(activeServerId, run)
    }
...
watch(selectedTestIds, (testIds) => {
  ...
  serverStore.saveHarnessTestSelection(serverId, testIds)
})
...
onMounted(() => {
  ...
  const preferences = loadHarnessPreferences()
