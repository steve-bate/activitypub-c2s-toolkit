...
import { allAssertionsPassed, evaluateAssertions } from '@/testing/core/assertions'
import { buildDependencyPlan } from '@/testing/core/dependencyGraph'
import { executeBrowserRequest, runSidecarDiagnostics, shouldAttemptSidecarDiagnostics } from '@/testing/core/executor'
import type {
  BrowserInfo,
  HarnessRunResult,
  TestAssertion,
...
function asRequestPreparation(test: TestCaseDefinition, context: TestCaseContext) {
  if (!test.request) {
    return undefined
  }
...
function calculateSummary(results: HarnessRunResult['results']): HarnessRunResult['summary'] {
  const summary = {
    pass: 0,
    fail: 0,
    error: 0,
  ...
  for (const result of results) {
    ...
    if ((result.status === 'skip' || result.status === 'inconclusive') && result.reason?.includes('Blocked by dependency')) {
      summary.blocked += 1
    }
  }
...
function withBearerTokenIfAvailable(request: TestRequest, options: TestRunOptions): TestRequest {
...
function parseBrowserNameAndVersion(userAgent: string): Pick<BrowserInfo, 'name' | 'version'> {
  const ua = userAgent || ''
...
export async function runTestSuite(
  ...
  onResult?: (result: HarnessRunResult['results'][number]) => void
  ...
  const results: HarnessRunResult['results'] = []
  ...
  for (const test of planningResult.order) {
    ...
    if (failedDependencies.length > 0) {
      const blockedStatus: TestStatus = blockedDependencyPolicy === 'inconclusive' ? 'inconclusive' : 'skip'
      ...
      outcome = await test.run(context, {
        executeRequest: async (request, assertions = []) => {
          ...
          if (shouldAttemptSidecarDiagnostics(attempt)) {
            diagnostics = await runSidecarDiagnostics(options.sidecarUrl, authedRequest, attempt, timeoutMs)
          }

          if (!attempt.exchange.success && assertions.length === 0 && attempt.transportError) {
            return {
              status: 'fail',
          ...
          const status: TestStatus = allAssertionsPassed(assertionOutcomes)
            ? 'pass'
            : (attempt.transportError ? 'error' : 'fail')
      ...
      if (!preparation) {
        ...
        const authedRequest = withBearerTokenIfAvailable(preparation.request, options)
        const attempt = await executeBrowserRequest(authedRequest, timeoutMs)
        const assertionOutcomes = evaluateAssertions(test.assertions ?? [], attempt)
        ...
        const status: TestStatus = assertionOutcomes.length > 0
          ? (allAssertionsPassed(assertionOutcomes) ? 'pass' : (attempt.transportError ? 'error' : 'fail'))
          : (attempt.exchange.success ? 'pass' : 'fail')

        outcome = {
          status,
          reason: status === 'pass' ? undefined : (attempt.exchange.error ?? 'Request failed'),
          assertions: assertionOutcomes,
          attempt,
          diagnostics
        }
  ...
  return {
    ...
    selectedTestIds: options.selectedTestIds && options.selectedTestIds.length > 0
...
function buildTools(options: TestRunOptions, timeoutMs: number) {
  return {
    executeRequest: async (request: TestRequest, assertions: TestAssertion[] = []) => {
      const authedRequest = withBearerTokenIfAvailable(request, options)
      const attempt = await executeBrowserRequest(authedRequest, timeoutMs)
      const assertionOutcomes = evaluateAssertions(assertions, attempt)
    ...
    inconclusive: (reason: string): TestCaseExecutionOutcome => ({ status: 'inconclusive', reason, assertions: [] }),
    fail: (reason: string): TestCaseExecutionOutcome => ({ status: 'fail', reason, assertions: [] }),
    pass: (reason?: string): TestCaseExecutionOutcome => ({ status: 'pass', reason, assertions: [] })
  }
}
...
export async function rerunSingleTest(
  ...
  tests: TestCaseDefinition[],
  options: TestRunOptions,
  priorResults: HarnessRunResult['results'],
  priorContext: Record<string, unknown>
): Promise<HarnessRunResult['results'][number]> {
  ...
  if (failedDependencies.length > 0) {
    outcome = buildBlockedOutcome('skip', `Blocked by dependency: ${failedDependencies.join(', ')}`)
  } else if (test.run) {
    outcome = await test.run(context, buildTools(options, timeoutMs) as Parameters<typeof test.run>[1])
  } else {
    const preparation = asRequestPreparation(test, context)
    if (!preparation) {
      outcome = { status: 'error', reason: 'Test has no run function or request definition', assertions: [] }
    } else if ('inconclusiveReason' in preparation) {
      outcome = { status: 'inconclusive', reason: preparation.inconclusiveReason, assertions: [] }
    } else {
      const authedRequest = withBearerTokenIfAvailable(preparation.request, options)
      const attempt = await executeBrowserRequest(authedRequest, timeoutMs)
      const assertionOutcomes = evaluateAssertions(test.assertions ?? [], attempt)
