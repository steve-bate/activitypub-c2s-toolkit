...
function createTestMap(tests: TestCaseDefinition[]): Map<string, TestCaseDefinition> {
...
function validateDependencies(tests: TestCaseDefinition[]): DependencyValidationResult {
...
function findCycleMembers(tests: TestCaseDefinition[]): string[] {
  ...
  const sortedIds = Array.from(adjacency.keys()).sort((a, b) => a.localeCompare(b))
