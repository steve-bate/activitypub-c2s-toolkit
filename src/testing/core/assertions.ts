import type { AssertionOutcome, BrowserAttemptResult, TestAssertion } from '@/testing/core/types'
...
function hasActivityPubContext(payload: unknown): boolean {
  ...
  if (Array.isArray(context)) {
    return context.some((item) => typeof item === 'string' && item.includes(expected))
  }

  return false
}

function evaluateAssertion(assertion: TestAssertion, attempt: BrowserAttemptResult): AssertionOutcome {
  ...
  switch (assertion.kind) {
    case 'status': {
      ...
      const passed = typeof expectedStatus === 'number' && actualStatus === expectedStatus
      return {
        assertion,
        passed,
        message: passed
    ...
    case 'jsonPathEquals': {
      ...
      const pathValue = getPathValue(payload, assertion.jsonPath)
      const passed = JSON.stringify(pathValue) === JSON.stringify(assertion.expectedValue)
    ...
    case 'activityPubObjectShape': {
      ...
      const hasType = typeof objectPayload?.type === 'string' && objectPayload.type.length > 0
      const hasId = typeof objectPayload?.id === 'string' && objectPayload.id.length > 0
      const hasContext = hasActivityPubContext(payload)
      const passed = hasType && hasContext && (hasId || assertion.expectedType === 'collection')
...
export function evaluateAssertions(assertions: TestAssertion[], attempt: BrowserAttemptResult): AssertionOutcome[] {
...
export async function getCollectionItemUris(
  ...
  const pageUri = firstRef && typeof firstRef === 'object' ? 
    (firstRef as Record<string, unknown>).id : firstRef
