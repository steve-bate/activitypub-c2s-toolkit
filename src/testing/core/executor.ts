...
export async function executeBrowserRequest(
  ...
  try {
    ...
    const exchange: HttpExchange<HttpRequestData<string>, HttpResponseData<unknown>> = {
      success: response.ok,
...
export function shouldAttemptSidecarDiagnostics(attempt: BrowserAttemptResult): boolean {
  ...
  return transportKind === 'transport' || transportKind === 'timeout' || transportKind === 'opaque'
}
...
export async function runSidecarDiagnostics(
  ...
  try {
    ...
    let summary = 'Browser transport failed; sidecar reached endpoint and fetched target URI'

    const corsState = responsePayload && typeof responsePayload === 'object'
      ? (responsePayload as Record<string, unknown>).cors
      : undefined

    if (corsState && typeof corsState === 'object') {
      const missingHeaders = (corsState as Record<string, unknown>).missingHeaders
