...
const tests: TestCaseDefinition[] = [
  {
    ...
    run: async (context, tools) => {
      ...
      const authenticatedActorUri = (
        context.parameters as Record<string, unknown>
      ).authenticatedActorUri as string | undefined
      ...
      if (actorUri !== authenticatedActorUri) {
        return tools.fail(
          `WebFinger resolved ${actorUri}, expected ${authenticatedActorUri}`,
        )
      }
  ...
  },
  {
    ...
    run: async (context, tools) => {
      const actorUri = (context.parameters as Record<string, unknown>)
        .authenticatedActorUri as string | undefined
      if (!actorUri) {
...
export const actorResolutionAndCollectionsSuite: TestSuiteDefinition = {
  ...
  description:
    "Validate WebFinger identity mapping and follow inbox/outbox dependency chains from authenticated actor URI.",
  tests,
}
