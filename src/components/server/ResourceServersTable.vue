      ...
      <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
        <tr v-if="serverStore.servers.length === 0">
          <td colspan="6" class="px-6 py-8 text-center text-gray-500 dark:text-gray-400">
            No servers configured yet. <RouterLink to="/servers/new" class="text-blue-600 dark:text-blue-400 hover:underline font-medium">Add one now</RouterLink>.
          </td>
        </tr>
        <tr v-for="server in serverStore.servers" :key="server.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
          ...
          <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300">
            {{ server.authType === 'oauth2' ? 'OAuth2' : 'Bearer' }}
          </td>
          <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300">
