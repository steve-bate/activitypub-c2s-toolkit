...
The repository includes a sidecar server that proxies outbound HTTP requests and reports CORS-related diagnostics. This can be useful since a browser will typically hide many details related to CORS failures, making it difficult for a developer to diagnose the reason for request failures.
...
Example `POST /diagnostics` JSON payload shape:
