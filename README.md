...
- 🧪 Built-in browser-first test harness with dependency-aware suite execution
...
Open the harness at `/tests` from the top navigation.
...
Run history is persisted in localStorage with bounded retention and can be exported as JSON from the harness page.
...
- Additional suites and assertion plugins for the test harness.
